...
class OxigenReaction(object):
	...
	def calculate(self,	sys=None, sys_O=None, sys_OH=None, sys_OOH=None, sys_O2=None, 
		...
		self.result = self.reaction( 	E={'*':sys, '*O':sys_O, '*OH':sys_OH, '*OOH':sys_OOH, '*O2':sys_O2, 'H2':H2, 'H2O':H2O}, T=T, v=False, save=True, 
						Gb=Gb, thermodimanic_corrections=thermodimanic_corrections )

		return self.result 

	@Logs.LogDecorator()
	def reaction(self, E={'*':0, '*O':0, '*OH':0, '*OOH':0, 'H2':0, 'H2O':0, '*O2':0}, T=298, 
	...
	def plot(self, data=None, ax=None, folder='.', name='', dpi=40, save=False, v=False):
		...
		if type(data) != type(None):
			self.plot_reactioncoordinate(
						...
						plot_limits={'None':None, 'Y':[-2.0, 0.5]}, step_dimentions=[2,0], save={'folder':folder, 'name':name, 'dpi':dpi, 'save':save} )
	...
	def plot_reactioncoordinate(self,	
					...
					OP_plot={'show':True, 'font_size':15}, save={'folder':'.', 'save':False},
		...
		try:
			if save['save']:
				...
				save_fig_folder  = save['folder'] if 'folder' in save and save['folder'] != None else './'
				save_fig_name    = save['name']   if 'name'   in save and save['name']   != None else label['title']
				save_fig_ext     = save['ext']    if 'ext'    in save and save['ext']    != None else 'png'
				save_fig_DPI     = save['dpi']    if 'dpi'    in save and save['dpi']    != None else 40

				print('{}/{}.{}'.format(save_fig_folder, save_fig_name, save_fig_ext))
				fig.savefig('{}/{}.{}'.format(save_fig_folder, save_fig_name, save_fig_ext), bbox_inches='tight', dpi=save_fig_DPI )
	...
	@Logs.LogDecorator()
	def plot_integrated(self, title='', save={'save':False}, v=True):
		...
		# === make PLOTs === #
		self.plot(ax=ax[0], save=True, v=True) 			 	# reaction coordinate plot
		self.plot_absortion(ax=ax[1], save=True, v=True)	# absoriot free energies 
		self.plot_table(ax=ax2, save=True, v=True)			# numeric table 
	...
	def summarise_absortion(self, latex=False, v=True):
		try:
			...
			print('ERROR :: ORR.summarise_absortion() :: Need to calculate ORR.reaction() ')
			return None
	...
	def cookbook(self, ):
		...
		def purvoix():
			...
			orr.calculate(	sys={'E':-832.979,'ZPE':0.0,'S':0.0}, sys_O={'E':-833.3057,'ZPE':0.07,'S':0.0}, 
...
'''
orr = OxigenReaction() # ========= FeTPyPZn Au D3 ========= # 
orr.calculate(sys={'E':-828.55712,'ZPE':0.0,'S':0.0}, sys_O={'E':-833.19239,'ZPE':0.07,'S':0.0},
			 sys_OH={'E': -838.14144,'ZPE':0.35,'S':0.0}, sys_OOH={'E':-842.61156,'ZPE':0.43,'S':0.0}, sys_O2=None, 
						H2O={'E':-14.213,'ZPE':0.56,'S':0.67}, H2={'E':-6.76,'ZPE':0.27,'S':0.41}, Gb=-0.0)
orr.plot_integrated('FeTPyPZn Au 13.7 D3')
orr.summarise_steps()
...
orr = OxigenReaction() # ========= FeTPyP + 0F FS sin comprimir D3 ========= # 
orr.calculate(sys={'E':-522.28948,'ZPE':0.0,'S':0.0}, sys_O={'E':-528.33233,'ZPE':0.07,'S':0.0},
			 sys_OH={'E':-532.71342,'ZPE':0.35,'S':0.0}, sys_OOH={'E':-537.01342,'ZPE':0.43,'S':0.0}, sys_O2=None, 
						H2O={'E':-14.213,'ZPE':0.56,'S':0.67}, H2={'E':-6.76,'ZPE':0.27,'S':0.41}, Gb=-0.0)
orr.plot_integrated('FeTPyP + 0F Au 13.7 D3 Norkov')
orr.summarise_steps()
orr.summarise_absortion()  


orr = OxigenReaction() # ========= FeTPyP + 0F FS sin comprimir D3 ========= # 
orr.calculate(sys={'E':-522.28948,'ZPE':0.0,'S':0.0}, sys_O={'E':-528.33233,'ZPE':0.07,'S':0.05},
			 sys_OH={'E':-532.71342,'ZPE':0.35,'S':0.07}, sys_OOH={'E':-537.01342,'ZPE':0.43,'S':0.16}, sys_O2=None, 
						H2O={'E':-14.213,'ZPE':0.56,'S':0.67}, H2={'E':-6.76,'ZPE':0.27,'S':0.41}, Gb=-0.0)
orr.plot_integrated('FeTPyP + 0F Au 13.7 D3 LEE')
orr.summarise_steps()
orr.summarise_absortion()  
plt.show()
...
orr = OxigenReaction() # ========= FeTPyP + 1F FS sin comprimir D3 ========= # 
orr.calculate(sys={'E':-521.78385,'ZPE':0.0,'S':0.0}, sys_O={'E':-528.31388	,'ZPE':0.07,'S':0.0},
			 sys_OH={'E':-532.18503,'ZPE':0.35,'S':0.0}, sys_OOH={'E':-537.16497,'ZPE':0.43,'S':0.0}, sys_O2=None, 
						H2O={'E':-14.213,'ZPE':0.56,'S':0.67}, H2={'E':-6.76,'ZPE':0.27,'S':0.41}, Gb=-0.0)
orr.plot_integrated('FeTPyP + 1F Au 13.7 D3')
orr.summarise_steps()
orr.summarise_absortion()  

orr = OxigenReaction() # ========= FeTPyP + 2F FS sin comprimir D3 ========= # 
orr.calculate(sys={'E':-522.18242,'ZPE':0.0,'S':0.0}, sys_O={'E':-528.13242,'ZPE':0.07,'S':0.0},
			 sys_OH={'E':-532.18503,'ZPE':0.35,'S':0.0}, sys_OOH={'E':-537.04143,'ZPE':0.43,'S':0.0}, sys_O2=None, 
						H2O={'E':-14.213,'ZPE':0.56,'S':0.67}, H2={'E':-6.76,'ZPE':0.27,'S':0.41}, Gb=-0.0)
...
orr = OxigenReaction() # ========= FeTPyP + 3F FS sin comprimir D3 ========= # 
orr.calculate(sys={'E':-522.22745,'ZPE':0.0,'S':0.0}, sys_O={'E':-528.1381,'ZPE':0.07,'S':0.0},
			 sys_OH={'E':-532.53805,'ZPE':0.35,'S':0.0}, sys_OOH={'E':-537.06447,'ZPE':0.43,'S':0.0}, sys_O2=None, 
						H2O={'E':-14.213,'ZPE':0.56,'S':0.67}, H2={'E':-6.76,'ZPE':0.27,'S':0.41}, Gb=-0.0)
orr.plot_integrated('FeTPyP + 3F Au 13.7 D3')
orr.summarise_steps()
orr.summarise_absortion()  

orr = OxigenReaction() # ========= FeTPyP + 6F FS sin comprimir D3 ========= # 
orr.calculate(sys={'E':-521.88576,'ZPE':0.0,'S':0.0}, sys_O={'E':-527.78198,'ZPE':0.07,'S':0.0},
			 sys_OH={'E':-532.18503,'ZPE':0.35,'S':0.0}, sys_OOH={'E':-536.49185,'ZPE':0.43,'S':0.0}, sys_O2=None, 
						H2O={'E':-14.213,'ZPE':0.56,'S':0.67}, H2={'E':-6.76,'ZPE':0.27,'S':0.41}, Gb=-0.0)
orr.plot_integrated('FeTPyP + 6F Au 13.7 D3')
orr.summarise_steps()
orr.summarise_absortion()  

orr = OxigenReaction() # ========= FeTPyP + 7F FS sin comprimir D3 ========= # 
orr.calculate(sys={'E':-521.6336,'ZPE':0.0,'S':0.0}, sys_O={'E':-527.59094,'ZPE':0.07,'S':0.0},
			 sys_OH={'E':-531.98105,'ZPE':0.35,'S':0.0}, sys_OOH={'E':-537.80282,'ZPE':0.43,'S':0.0}, sys_O2=None, 
						H2O={'E':-14.213,'ZPE':0.56,'S':0.67}, H2={'E':-6.76,'ZPE':0.27,'S':0.41}, Gb=-0.0)
...
if __name__ == "__main__":
	# ========================== ????  FS  ========================== #
	orr = OxigenReaction() # ========= ???? D3 ========= # 
	orr.calculate(sys={'E':-531.313,'ZPE':0.0,'S':0.0}, sys_O={'E':-528.4696,'ZPE':0.07,'S':0.0},
				 sys_OH={'E':-541.721,'ZPE':0.35,'S':0.0}, sys_OOH={'E':-537.929,'ZPE':0.43,'S':0.0}, sys_O2=None, 
							H2O={'E':-14.213,'ZPE':0.56,'S':0.67}, H2={'E':-6.76,'ZPE':0.27,'S':0.41}, Gb=-0.0)
	orr.plot_integrated('???  13.7 (M2) D3'.format(0))
	orr.summarise_steps()
	orr.summarise_absortion()
	plt.show()
	...
	# ========================== ZnTPyP FS  ========================== #
	orr = OxigenReaction() # ========= ZnTPyP D3 ========= # 
	orr.calculate(sys={'E':-416.38958,'ZPE':0.0,'S':0.0}, sys_O={'E':-419.30339,'ZPE':0.07,'S':0.0},
				 sys_OH={'E':-425.52925,'ZPE':0.35,'S':0.0}, sys_OOH={'E':-430.162,'ZPE':0.43,'S':0.0}, sys_O2=None, 
							H2O={'E':-14.213,'ZPE':0.56,'S':0.67}, H2={'E':-6.76,'ZPE':0.27,'S':0.41}, Gb=-0.0)
	orr.plot_integrated('ZnTPyP  13.7 (M2) D3'.format(0))
	orr.summarise_steps()
	orr.summarise_absortion()
	plt.show()

	# ========================== NiTPyP FS  ========================== #
	orr = OxigenReaction() # ========= NiTPyP D3 ========= # 
	orr.calculate(sys={'E':-420.98085,'ZPE':0.0,'S':0.0}, sys_O={'E':-423.98876,'ZPE':0.07,'S':0.0},
				 sys_OH={'E':-429.84392,'ZPE':0.35,'S':0.0}, sys_OOH={'E':-434.58642,'ZPE':0.43,'S':0.0}, sys_O2=None, 
							H2O={'E':-14.213,'ZPE':0.56,'S':0.67}, H2={'E':-6.76,'ZPE':0.27,'S':0.41}, Gb=-0.0)
	orr.plot_integrated('NiTPyP  13.7  D3'.format(0))
	orr.summarise_steps()
	orr.summarise_absortion()
	plt.show()
	...
	# ========================== ZnTPyP  ========================== #
	orr = OxigenReaction() # ========= ZnTPyP D3 ========= # 
	orr.calculate(sys={'E':-515.89408,'ZPE':0.0,'S':0.0}, sys_O={'E':-518.79876,'ZPE':0.07,'S':0.0},
				 sys_OH={'E':-524.88368,'ZPE':0.35,'S':0.0}, sys_OOH={'E':-529.71671,'ZPE':0.43,'S':0.0}, sys_O2=None, 
							H2O={'E':-14.213,'ZPE':0.56,'S':0.67}, H2={'E':-6.76,'ZPE':0.27,'S':0.41}, Gb=-0.0)
	orr.plot_integrated('ZnTPyP  13.7 (M2) D3'.format(0))
	orr.summarise_steps()
	orr.summarise_absortion()
