...
class XML(object):
	...
	@Logs.LogDecorator()
	def summary(self, 	path:str=None,  metadata:dict=None, analysis:list=['ORR', 'position', 'magnetization', 'metadata'], 
						save:bool=True,  ):
		path = path if not path is None else self.path
		# metadata =  { 'sistema_i': {propiedad_m:value} }
		# fill the workbook with all possible data from DATA.py #
		def make_paths(metadata:dict=None, analysis:list=None):
			if 'meta-analysis' in analysis:
				for key0, item0 in metadata.items:
					for key1, item1 in metadata.items:
						self.make_path(f'{path}/plots/metanalisis/{key1}')
			
			if 'ORR' in analysis:
				self.make_path(f'{path}/plots/metanalisis/ORR')
				
			if 'magnetization' in analysis:
		...
		# ====== Create folthers ====== #
		make_paths(metadata=metadata, analysis=analysis)

		# ====== GET/ADD data ====== #
		if 'ORR' in analysis:
			# -- get ORR data -- #
			self.get_ORR_array(metadata=metadata, analysis=analysis)
			self.get_ORR_DF(metadata=metadata, analysis=analysis)
			# -- add ORR data -- #
			self.add_ORR_sheet()
			#self.summary_ORR()
	...
	def summary_set(self, set_full=None, path=None, plot=False, WB=None):
	...
	def summary_data(self, data, path=None, plot=True, key_data=None, WB=None):
		...
		if self.ORR_activated:
			try:
				...
				# == make plot == # 
				if not os.path.exists(f"{path}/plots/metanalisis/ORR/{key_data}.png"):
					ORR.plot(folder=f'{path}/plots/metanalisis/ORR', name=key_data)

				# == if exist add plot to sheet == #
				if os.path.exists(f"{path}/plots/metanalisis/ORR/{key_data}.png"):
					self.plot_add(	path  =f"{path}/plots/metanalisis/ORR", 
									name  =f"{key_data}.png", 
									anchor=f'{get_column_letter(1)}1', 
									sheet =sheet, height=400, width=400)		
				else:
					print(f'WARNNING :: Can not add {path}/plots/metanalisis/ORR/{key_data}.png')
			except: pass
			
		# == set some sheer variables == #
		#sheet.column_dimensions['A'].width = 30
		for system_n, (key_system, system) in enumerate(data.system.items()):
			self.summary_system(system=system, key_system=key_system, key_data=key_data,  
	...
	def summary_system(self, system, key_system=None,  key_data=None,
		...
		try:
			if not os.path.exists(f'{path}/plots/systemas/{key_data}/{key_system}.png'):
				try:
					# >> PLOT atoms positions from CONTCAR/POSCAR
					if plot and not system.CONTCAR is None:		system.CONTCAR.plot(save=True, path=f'{path}/plots/systemas/{key_data}/{key_system}.png')
					# >> if CONTCAR/POSCAR is not available plot from OUTCAR
					elif plot and not system.OUTCAR is None:	system.OUTCAR.plot_poscar(save=True, path=f'{path}/plots/systemas/{key_data}/{key_system}.png')
					#pass # !!!!!!!!!!!!!!!
				except Exception:
					traceback.print_exc()
					print(f'ERROR :: can not plot system {key_system} >> {traceback.print_exc()}')
	...
	# === === === ORR summary === === === # # === === === ORR summary === === === #  # === === === ORR summary === === === #  # === === === ORR summary === === === # 
	# === === === ORR summary === === === # # === === === ORR summary === === === #  # === === === ORR summary === === === #  # === === === ORR summary === === === # 
	def summary_ORR(self, 	metadata:dict=None, analysis:list=None,
							ORR_df=None, path=None, WB=None):
	...
	# === === === ORR getter === === === # 
	def get_ORR_array(self, sSet:dict=None, ORR_relevant=None, 
							metadata:dict=None, analysis:list=None,
							plot:bool=True, path:str=None, save:bool=True, ):

		# ---- set conteiners ---- #
		path         = path 	 	if not path 			is None else self.path
		sSet     	 = sSet    		if not sSet 			is None else self.set.set
		ORR_relevant = ORR_relevant if not ORR_relevant 	is None else self.ORR_relevant

		ORR_array   = []
		for set_n, (key_data, data) in enumerate(sSet.items()):
			# -- generate OBJ and analize the ORR reaction -- #
			try:
				data.ORR()
				ORR = data.reaction('ORR')

				# -- generate plot -- #
				if plot and not os.path.exists(f"{path}/plots/metanalisis/ORR/{key_data}.png"):
					try:	ORR.plot_integrated(title=key_data, save={'save':True, 'folder':f'{path}/plots/metanalisis/ORR/.', 'name':key_data, 'dpi':100} )
					except: pass
					#ORR.plot(save=f'{path}/plots/metanalisis/ORR/.', name=key_data, save=True)

				# -- store DATA in vector-- #
				if type(ORR.ORR) == dict:	 # store ORR info of all systems #
					vector_labels  = [key_data] + [m for m in metadata[key_data] ] if not metadata is None else [key_data]
	...
	def get_ORR_DF(self, metadata:dict=None, analysis:list=None,
						 ORR_array=None    , ORR_relevant=None, 
						 save=True):
		ORR_array    = ORR_array    if not ORR_array        is None else self.ORR_array
		ORR_relevant = ORR_relevant if not ORR_relevant 	is None else self.ORR_relevant

		# -- make ORR dataframe -- #
		if not metadata is None:		ORR_df  	= pd.DataFrame(data=ORR_array,  columns=ORR_relevant+['FullID']+[m for m in metadata.items()[0][1] ]) # store ORR info of all systems #
		else:							ORR_df  	= pd.DataFrame(data=ORR_array,  columns=ORR_relevant+['FullID'])
		ORR_df[ORR_relevant] = ORR_df[ORR_relevant].apply(pd.to_numeric)
		if save:	self.ORR_df = ORR_df

		return ORR_df
	
	# === === === Magnetization getter === === === # # === === === Magnetization getter === === === # # === === === Magnetization getter === === === # # === === === Magnetization getter === === === # 
	# === === === Magnetization getter === === === # # === === === Magnetization getter === === === # # === === === Magnetization getter === === === # # === === === Magnetization getter === === === # 
	def get_Magnetization_array(self, 	metadata:dict=None, analysis:list=None,
										save:bool=True, plot:bool=True):
		# set conteiners #
		Magnetization_array = []
		...
		for set_n, (key_data, data) in enumerate(self.set.set.items()):
			...
			for system_n, (key_system, system) in enumerate(data.system.items()):
				# -- store DATA in vector-- #
				print( system.OUTCAR, system.loaded )
				if not system.OUTCAR is None:
					try:
						atoms_names =  np.array(system.OUTCAR.atoms_names_list)[system.OUTCAR.relevant_atoms_mask()] 
						atoms_magnetization = np.abs(system.OUTCAR.resume()['dict']['magnetization'][system.OUTCAR.relevant_atoms_mask()][:,-1])
						Magnetization_array += [ vector_labels+[key_system, atoms_names[atom_i], atom_i, atoms_magnetization[atom_i]] for atom_i, atom_name in enumerate(atoms_names)  ]	
					except: pass
	...
	def get_Magnetization_DF(self, 	metadata:dict=None, analysis:list=None,
									Magnetization_array=None, save:bool=True):
		Magnetization_array = Magnetization_array if not Magnetization_array is None else self.Magnetization_array
		print(Magnetization_array)
		# -- make MARNETIZATION dataframe -- #
		Magnetization_array = np.array(Magnetization_array)
		if not metadata is None:		Magnetization_df  	= pd.DataFrame(data=Magnetization_array,  columns= ['FullID'] + [m for m in metadata.items()[0][1]] + ['System', 'Atom', 'number', 'Magnetization'] ) # store ORR info of all systems #
		else:							Magnetization_df  	= pd.DataFrame(data=Magnetization_array,  columns= ['FullID', 'System', 'Atom', 'number', 'Magnetization'] ) # store ORR info of all systems #
	...
	# === === === Distance getter === === === # # === === === Distance getter === === === # # === === === Distance getter === === === # # === === === Distance getter === === === # 	
	# === === === Distance getter === === === # # === === === Distance getter === === === # # === === === Distance getter === === === # # === === === Distance getter === === === # 	
...
xml = XML(path='.')
xml.set_load(filename='/home/akaris/Documents/code/VASP/v4.7/files/dataset/metales/dataset_Metales_MTPyP_catalysis_part2.pkl')
#xml.set_load(filename='/home/akaris/Documents/code/VASP/v4.7/files/dataset/catalisis/dataset_CoFeTPyPCoFe_catalysis.pkl')
xml.summary()
