...
class POSCAR(object):
	...
	def b(self):
		if self._b is None:
			self._b = np.zeros_like(self.cell) 
			self._b[0,:] = 2*np.pi/self.volumen() * np.cross( self.a[1,:], self.a[2,:]) * self.scale*self.scale
			self._b[1,:] = 2*np.pi/self.volumen() * np.cross( self.a[2,:], self.a[0,:]) * self.scale*self.scale
			self._b[2,:] = 2*np.pi/self.volumen() * np.cross( self.a[0,:], self.a[1,:]) * self.scale*self.scale
	...
	def load(self, file_name:str=None, save:bool=True) -> list:
		...
		for i, n in enumerate(f):
			...
			if i >= 9-shift:
				if i < N+9-shift: 
					atoms[i-9+shift,:] = np.array( [vec[0], vec[1], vec[2]] )
	...
	def volumen(self, ): return self.get_volumen()

	def get_volumen(self, ):
		# returns the cell volumen  
		# ------------------------------------------------------
		# 
		return np.abs(np.dot(self.cell[0,:]*self.scale, np.cross(self.cell[1,:]*self.scale, self.cell[2,:]*self.scale)))
	...
	def plot(self, save=False, path='./no_name.png'):
		if type(self.atoms) != type(None):
			...
			for itr, a in enumerate(ax.flatten()):
				plot_atoms(atoms, a, rotation=rot[itr], show_unit_cell=True, )
				a.axis('off')
	...
	@Logs.LogDecorator()
	def export_PDB(self, name, atoms=None, atoms_names_list=None, bond_distance=2.3, v=True):
	...
	def get_ase(self, ):
		return Atoms(	self.atoms_names_list,
			 positions=	self.atoms*self.scale,
			 cell     =	self.cell*self.scale,
			 pbc      =	[1, 1, 0] )
...
# *** EG *** #
pos = POSCAR()
pos.load(file_name = '/home/akaris/Documents/code/VASP/v4.7/files/POSCAR/M1TPyM2/M1TPyPM2s_FS/O/FeTPyPFe_2')
