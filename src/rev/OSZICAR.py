...
class OSZICAR(object):
	...
	def load(self, file_name=None, save=True, low_mem=False):
		...
		except: 	print('ERROR :: OSZICAR.load() :: missing file {}'.format(file_name) ); 	return None
	...
	def export(self, file:str=None):
		file = file if not file is None else 'OSZICAR'

		try: 	f = open(file, 'w')
		except: 	print('ERROR :: OSZICAR.load() :: missing file {}'.format(file) ); 	return None

		f.write(f' {self.N} :: E {self.E} MAG {self.mag} F {self.F}')
		f.close()
