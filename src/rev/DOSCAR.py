...
class DOSCAR(object):
	...
	def cut(self, ion=0, orbital=0, 
		...
		if self.E is None or self.fermi is None: 			return None 
	...
	def get_center_of_band(self, ion:list=None, orbital:list=None,  add_fermi:bool=True, sum_plot:bool=False, save:bool=True, v:bool=True):
		# === get data === # 
...
'''
doscar = DOSCAR()
doscar.load( file_name='/home/akaris/Documents/code/VASP/v4.7/files/DOSCAR/CoFeTPyP/DOSCAR' )

doscar.plot_orbital_diagram(ion=[65], orbital=['d'], add_fermi=True)
doscar.plot( orbital=['d'] )
doscar.get_occupancy(ion=[65], orbital=['d'], add_fermi=True)
plt.show()
