...
class POTCAR(object):
	def __init__(self, file_name=None, atoms=None, atoms_list=None, name=None):
		self.name = name
		self.path = None
	...
	def copy(self, src:str=None, dst:str=None):
		if type(src) == type(None):	 src = self.file_name
		import shutil
	...
	def save(self, file:str=None):
		self.copy(src=self.file_name, dst=file)

if __name__ == "__main__":
	# *** eg. usage:
	potcar = POTCAR()
	potcar.load(file_name='/home/akaris/Documents/code/VASP/v4.7/files/POTCARs', v=False)
	potcar.summary()
	''' 
	'''
