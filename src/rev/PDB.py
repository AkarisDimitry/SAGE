# *** warning supresion
import warnings, os, time, argparse, sys
...
class PDB(object): # generador de datos
	...
	def READ(self, path=None, name=None, save=True, v=True):
		...
		if save:
			...
			self.HETATM_xyz   = np.array(HETATM_xyz)	# 31 - 38        Real(8.3)     x            Orthogonal coordinates for X in Angstroms.
														# 39 - 46        Real(8.3)     y            Orthogonal coordinates for Y in Angstroms.
														# 47 - 54        Real(8.3)     z            Orthogonal coordinates for Z in Angstroms.
	...
	def aling2pdb(self, PDB:object, atoms:list=None, output:str='pdb.pdb', save:bool=True) -> np.ndarray:

		ATOM1_xyz =  self.ATOM_xyz
		ATOM2_xyz =  PDB.ATOM_xyz
		N = ATOM1_xyz.shape[0]

		order = np.zeros((N))
		for i1, n1 in enumerate(ATOM1_xyz):
			sorted_norm1 =  np.sort(np.linalg.norm(n1-ATOM1_xyz, axis=1)) 
			sorted_dist_min = 999999 

			for i2, n2 in enumerate(ATOM2_xyz):
				sorted_norm2 =  np.sort(np.linalg.norm(n2-ATOM2_xyz, axis=1)) 
				sorted_dist = np.sum( (sorted_norm1 - sorted_norm2)**2 )

				if sorted_dist_min > sorted_dist:
					sorted_dist_min = sorted_dist
					order[i1] = i2

		ATOM3_xyz = np.zeros_like(ATOM2_xyz)
		for i, o in enumerate(order):
			ATOM3_xyz[int(o),:] = ATOM1_xyz[i,:]
		
		if save: 	self.export_PDB(str(output), atoms=ATOM3_xyz, atoms_names_list=PDB.ATOM_idx, connectivity=PDB.connectivity, sing=False, v=True)
...
def main(argv):
	...
	if task == 'export':
		# console INPUT example 
		# python3 PDB.py -t export -i /home/akaris/Documents/temporal/denise/3spu_A_sinOH.pdb, /home/akaris/Documents/temporal/denise/3spu_A_sinOH.pdb -o denise.pdb
	...
	if task == 'aling2pdb':
		# console INPUT example 
		# python3 PDB.py -t aling2pdb -i /home/akaris/Documents/uno.pdb /home/akaris/Documents/dos.pdb -o /home/akaris/Documents/tresb.pdb
		...
		# === aling & save === #
		PDB1.aling2pdb(PDB=PDB2, output=f'{outputfile}')
